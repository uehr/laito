...
class cliConverter:
    ...
    def fitedTerminalImageSize(self, img, margin=0):
        imgHeight, imgWidth = img.shape[:2]

        terminalWidth, terminalHeight = self.terminalSize()
        terminalWidth -= margin
        terminalHeight -= margin
    ...
    # img: numpy画像
    # 画像をTerminalImageへ変換
    def imageToTerminalImage(self, img, height=None, width=None, margin=0):
        imgHeight, imgWidth = img.shape[:2]

        if height is None or width is None:
            terminalImageSize = self.fitedTerminalImageSize(img, margin=margin)
        else:
